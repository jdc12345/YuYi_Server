...
-(void)textViewShowView:(UIViewController *)viewController customKeyboardDelegate:(id)delegate
{
    ...
    self.mBackView =[[UIView alloc]initWithFrame:CGRectMake(0, kScreenH, kScreenW, 50)];
    NSLog(@"%p",self.mBackView);
    self.mBackView.backgroundColor = [UIColor whiteColor];//[UIColor colorWithRed:229/255.0 green:229/255.0 blue:229/255.0 alpha:1];
    [self.mViewController.view addSubview:self.mBackView];
    NSLog(@"高度 %g",self.mViewController.view.frame.size.height);
    
    self.mSecondaryBackView =[[UIView alloc]initWithFrame:CGRectMake(10, 10, 230, 30)];
    self.mSecondaryBackView.backgroundColor = [UIColor whiteColor];//[UIColor colorWithRed:153/255.0 green:154/255.0 blue:158/255.0 alpha:1];
    [self.mBackView addSubview:self.mSecondaryBackView];
    
    self.mTextView =[[UITextView alloc]initWithFrame:CGRectMake(1, 1 +5, kScreenW -92, 28)];
    ...
    self.mTalkBtn.frame =CGRectMake(kScreenW -8 -34, 16, 34, 18);
    self.mTalkBtn.titleLabel.font = [UIFont systemFontOfSize:15];
    [self.mTalkBtn setTintColor:[UIColor colorWithHexString:@"#8e8e93"]];
...
- (BOOL)textViewShouldBeginEditing:(UITextView *)textView
{
    ...
    [self.mTalkBtn setTitleColor:[UIColor colorWithHexString:@"#8e8e93"] forState:UIControlStateNormal];
    return YES;
}
- (void) textViewDidChange:(UITextView *)textView{
    ...
    if ([textView.text length] == 0) {
        ...
        [self.mTalkBtn setTitleColor:[UIColor colorWithHexString:@"#448aff"] forState:UIControlStateNormal];
    }
}
...
- (void)keyboardWillShow:(NSNotification*)notification //键盘出现
{
    ...
    if (!self.mHiddeView)
    {
        self.mHiddeView =[[UIView alloc]initWithFrame:CGRectMake(0, 0,kScreenW,kScreenW)];
    ...
    [UIView animateWithDuration:0.05 animations:^{
        ...
        self.mBackView.frame =CGRectMake(0, kScreenH -_keyboardRect.size.height -50, kScreenW, 50);
...
#pragma mark 恢复的位置
        ...
        self.mBackView.frame=CGRectMake(0, kScreenH, kScreenW, 50);
        self.mHiddeView.alpha =0.0f;
