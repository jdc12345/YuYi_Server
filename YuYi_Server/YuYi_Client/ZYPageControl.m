...
- (void)initialization
{
    self.dotViewClass           = [ZYAnimatedDotView class];
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (touch.view != self) {
        ...
        if ([self.delegate respondsToSelector:@selector(ZYPageControl:didSelectPageAtIndex:)]) {
            [self.delegate ZYPageControl:self didSelectPageAtIndex:index];
        }
    }
}
...
/**
 *  Will update dots display and frame. Reuse existing views or insZYntiate one if required. Update their position in case frame changed.
 */
- (void)updateDots
{
...
- (UIView *)generateDotView
{
    ...
    if (self.dotViewClass) {
        ...
        if ([dotView isKindOfClass:[ZYAnimatedDotView class]] && self.dotColor) {
            ((ZYAnimatedDotView *)dotView).dotColor = self.dotColor;
        }
    } else {
...
/**
 *  Change activity sZYte of a dot view. Current/not currrent.
 *
 *  @param active Active sZYte to apply
 *  @param index  Index of dot for sZYte update
 */
- (void)changeActivity:(BOOL)active atIndex:(NSInteger)index
{
    if (self.dotViewClass) {
        ZYAbstractDotView *abstractDotView = (ZYAbstractDotView *)[self.dots objectAtIndex:index];
        if ([abstractDotView respondsToSelector:@selector(changeActivitySZYte:)]) {
            [abstractDotView changeActivityState:active];
        } else {
            NSLog(@"Custom view : %@ must implement an 'changeActivitySZYte' method or you can subclass %@ to help you.", self.dotViewClass, [ZYAbstractDotView class]);
        }
